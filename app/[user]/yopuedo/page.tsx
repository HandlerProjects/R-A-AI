...
export default function YoPuedoPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      loadDetalles(resolvedUserId),
      loadSueno(resolvedUserId),
    ]).then(([d, s]) => {
      setDetalles(d);
      setSueno(s);
      setSuenoDraft(s);
  ...
  useEffect(() => {
    const id = setInterval(() => setHintIndex((i) => (i + 1) % EXAMPLE_HINTS.length), 4000);
    return () => clearInterval(id);
  }, []);

  const addDetalle = async () => {
    const text = inputText.trim();
    if (!text || !resolvedUserId) return;
  ...
  const handleSaveSueno = async () => {
    const text = suenoDraft.trim();
    setSueno(text);
    setEditingSueno(false);
    if (resolvedUserId) await saveSueno(resolvedUserId, text);
  };

  const todayDetalles = detalles.filter((d) => d.date === todayStr());
  const olderDetalles = detalles.filter((d) => d.date !== todayStr());
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ messages: newMessages, userId: resolvedUserId, userName: userParam, module: "yopuedo" }),
      ...
      if (err?.name !== "AbortError") {
        setMessages((prev) => {
          const updated = [...prev];
          if (updated[updated.length - 1]?.role === "assistant" && !updated[updated.length - 1].content) updated.pop();
          return updated;
        });
      }
    } finally {
    ...
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "var(--bg-primary)", overflow: "hidden" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <h1 style={{ fontSize: 20, fontWeight: 700, color: "var(--text-primary)", margin: 0, letterSpacing: "-0.3px" }}>Yo puedo ✨</h1>
      ...
      <AnimatePresence mode="wait">
        ...
        {tab === "hoy" ? (
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
                <div style={{ flex: 1, position: "relative" }}>
                  <AnimatePresence mode="wait">
                    <motion.p key={hintIndex}
                      initial={{ opacity: 0, y: 4 }} animate={{ opacity: inputText ? 0 : 0.55 }} exit={{ opacity: 0, y: -4 }}
                      transition={{ duration: 0.4 }}
                      style={{ position: "absolute", top: 10, left: 12, fontSize: 13, color: "white", margin: 0, pointerEvents: "none", lineHeight: 1.4, zIndex: 1 }}
                    >
                  ...
                  <textarea
                    ...
                    onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); addDetalle(); } }}
                    rows={2}
                    style={{
                      width: "100%", background: "rgba(255,255,255,0.18)",
                      border: "1px solid rgba(255,255,255,0.3)", borderRadius: 12,
                      padding: "10px 12px", fontSize: 13, color: "white",
                      resize: "none", outline: "none", fontFamily: "inherit", boxSizing: "border-box",
                    }}
                  />
                </div>
                <motion.button whileTap={{ scale: 0.92 }} onClick={addDetalle} disabled={!inputText.trim()}
                  style={{
            ...
            <AnimatePresence>
              {justAdded && (
                ...
                >
                  <motion.span animate={{ scale: [1, 1.3, 1] }} transition={{ duration: 0.4 }} style={{ fontSize: 18 }}>💗</motion.span>
                  <p style={{ fontSize: 13, color: "#34C759", fontWeight: 500, margin: 0 }}>Guardado. Eso cuenta, de verdad.</p>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            {/* Loading */}
            {loadingDetalles && (
              <div style={{ textAlign: "center", padding: "20px 0", color: "var(--text-quaternary)", fontSize: 13 }}>Cargando…</div>
            )}

            {/* Today */}
            {!loadingDetalles && todayDetalles.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 18 }}>
                <p style={{ fontSize: 11, fontWeight: 700, color: "var(--text-quaternary)", textTransform: "uppercase", letterSpacing: "0.09em", marginBottom: 8 }}>De hoy</p>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {todayDetalles.map((d, i) => (
                    <motion.div key={d.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                      style={{
                        background: "white", border: "1px solid rgba(175,82,222,0.12)",
            ...
            {/* Older */}
            {!loadingDetalles && olderDetalles.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} style={{ marginBottom: 18 }}>
                <p style={{ fontSize: 11, fontWeight: 700, color: "var(--text-quaternary)", textTransform: "uppercase", letterSpacing: "0.09em", marginBottom: 8 }}>Otros días</p>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {olderDetalles.slice(0, 10).map((d, i) => (
                    <motion.div key={d.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 + i * 0.04 }}
                      style={{
                        background: "rgba(175,82,222,0.04)", border: "1px solid rgba(175,82,222,0.08)",
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              style={{ background: "white", border: "1px solid rgba(0,0,0,0.07)", borderRadius: 20, padding: "18px 18px", boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                ...
                <button onClick={() => { setEditingSueno(true); setSuenoDraft(sueno); }}
              ...
              <AnimatePresence mode="wait">
                {editingSueno ? (
                  <motion.div key="edit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <textarea autoFocus value={suenoDraft} onChange={(e) => setSuenoDraft(e.target.value)}
                      placeholder="Un perro salchicha, una casa con mi novio, viajar a Japón sin mirar el precio…"
                      rows={3}
                      style={{
                        width: "100%", background: "rgba(175,82,222,0.05)", border: "1px solid rgba(175,82,222,0.2)",
                        borderRadius: 12, padding: "10px 12px", fontSize: 13, color: "var(--text-primary)",
                        resize: "none", outline: "none", fontFamily: "inherit", boxSizing: "border-box", lineHeight: 1.4,
                      }}
                    />
                    <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                      <button onClick={() => setEditingSueno(false)}
                      ...
                      >Cancelar</button>
                      <button onClick={handleSaveSueno}
                  ...
                  <motion.p key="show" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    style={{ fontSize: 15, fontWeight: 500, color: ACCENT, margin: 0, lineHeight: 1.5, fontStyle: "italic" }}
                  >"{sueno}"</motion.p>
                ) : (
                  <motion.p key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    style={{ fontSize: 13, color: "var(--text-quaternary)", margin: 0, fontStyle: "italic" }}
                  >Sin límites de dinero ni de tiempo — ¿qué quieres?</motion.p>
                )}
              </AnimatePresence>
            </motion.div>
          ...
          >
            <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: "16px 16px 8px", display: "flex", flexDirection: "column", gap: 2 }}>
              {messages.map((msg, i) => (
      ...
      <AnimatePresence>
        {particles.map((p) => (
          <motion.div key={p.id}
            ...
            style={{ position: "fixed", bottom: "30%", left: `${p.x}vw`, fontSize: p.size, pointerEvents: "none", zIndex: 999, userSelect: "none" }}
          >{p.emoji}</motion.div>
        ))}
      </AnimatePresence>
